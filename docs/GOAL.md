...
M0 Phase 1 Workspace/Worktree、M1 Terminal + SSH 稳定性与 Phase 2 Markdown/单文件轻编辑均已完成。Phase 3 已按[Preview 安全规格](./PHASE3_PREVIEW_SOURCE_CONTRACT.md)建立来源身份、检测 allowlist、去重/stale 合同、隔离的 localhost WebView/navigation policy、opening/loading/ready/failed/closed/stale 的最小运行时状态与手动服务恢复闭环，以及可信 main Inspector 中的同源地址导航、原生前进/后退历史、有限原生缩放、常用 viewport、bounded console/network 失败摘要与绑定物理 PTY 送回、来源 terminal generation 绑定的 fail-closed 服务重启准备入口，以及用户显式、来源绑定、fail-closed 的 SSH remote loopback tunnel。截图仍是未满足的 Phase 3 required gate；保持后置，不自动进入 Phase 4。
