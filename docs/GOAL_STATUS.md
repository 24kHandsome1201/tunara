...
- [ ] Claude Code、Codex、Pi、OpenCode、Aider 和未知 TUI 的本地/SSH 兼容矩阵。TTY 基础层已形成可重复 smoke：5 个本地 Agent 与 5 个 SSH Agent 均完成启动 + resize，未知 TUI 的本地/SSH 完整协议合约通过；8/10 真实 Agent 格观察到 bracketed 多行 marker，5/10 在 fallback interrupt 前正常退出。provider 快照 4/10 完成，3 格确认真实 tool event；本地 Codex 首次目录信任提示已观察，本地 Claude/本地 Codex/SSH Codex 的明确 ID resume 已通过。SSH Aider 的显式 history 身份已落盘，但当前 provider key 无法认证；OpenCode 两端均已确认进入同一专用 session，但本地 timeout、SSH 凭据错误，三者都不计 resume 通过。安装版 Tunara 1.14.0 内的未知 TUI 另已通过 50/50 分屏 resize、后台切回、可见输出复制和中断后 shell 恢复。其余逐 Agent 权限/确认、失败/resume、分屏、后台与复制粘贴证据仍未完成，见 [兼容性基线](./benchmarks/m1-agent-tui-compatibility-2026-07-11.md)。
