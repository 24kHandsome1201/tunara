...
- [ ] SSH 安全写内核与断线/权限/冲突故障注入。typed `RemoteWriteIo` 事务、最多 16 个排他同目录临时名、写入/flush/mode/sync/close、替换前 fingerprint 复查、同连接 `mv -f --`、替换后 bytes+mode 验证、弱引用同路径锁及 17 类 fault matrix 均已落地。`outcomeUnknown` 现在携带 cleanupPending，前端严格解析 fingerprint/mode/token，未知期间禁用重复保存、保留草稿，并提供重连后的“确认远端结果”动作；Saved/Conflict 对账重新进入既有状态。新增默认 ignored 的真实生产 adapter 测试已在 `de-netcup` 通过：中文空格路径 `before→after`、`0640` 保持、同尺寸外改冲突不覆盖、零临时残留并清理夹具。全量 Rust 166 通过（3 ignored），真实 adapter 1/1、严格 clippy、完整 Node 520 通过（3 skipped）、typecheck、lint 与 production build 通过。仍需真实替换中断后的 UI 对账和两个独立 Tunara 进程竞争，故本项保持未完成。
