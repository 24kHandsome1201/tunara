...
- [ ] SSH 安全写内核与断线/权限/冲突故障注入。typed `RemoteWriteIo` 事务、最多 16 个排他同目录临时名、写入/flush/mode/sync/close、替换前 fingerprint 复查、同连接 `mv -f --`、替换后 bytes+mode 验证、弱引用同路径锁及 fault matrix 均已落地。临时文件准备完成后，以完整目标路径 SHA-256 派生固定长度同目录 lock dir，SFTP 原子 `mkdir` 将二次 fingerprint 检查与 rename 包进跨进程短临界区；10 分钟 stale 回收避免崩溃后永久锁死，释放失败降级为可对账 unknown。`outcomeUnknown` 前端严格解析并禁止盲目重试。真实 `de-netcup` 生产 adapter 继续通过中文空格路径、`0640`、外改冲突与零残留；新增两条完全独立 SSH/SFTP 连接连续 12 轮竞争，每轮严格 1 Saved + 1 Conflict，不共享应用内 mutex。仍需真实替换中断后的 UI 对账，故本项保持未完成。
