...
- [ ] SSH 安全写内核与断线/权限/冲突故障注入。typed transaction、跨进程原子 replace lock、10 分钟 stale 回收、unknown 前端禁止盲目重试及 fault matrix 均已落地。真实 `de-netcup` 生产 adapter 通过中文空格路径、`0640`、外改冲突与零残留；两条独立 SSH/SFTP 连接连续 12 轮竞争，每轮严格 1 Saved + 1 Conflict。新增可控 TCP proxy 与实例级 test-only exec 探针：服务端接受生产 `mv` 后暂停响应，独立连接证明远端已是新内容再切 TCP；原事务真实返回 OutcomeUnknown/cleanupPending，新连接复用从 Tauri command 抽出的生产 reconcile 核心得到 Saved。断线 residue 被明确观察并由测试隔离清理。仍需 GUI 重连点击和按 owner 自动清理本次 temp/lock，故本项保持未完成。
