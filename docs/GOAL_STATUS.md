...
- [ ] Claude Code、Codex、Pi、OpenCode、Aider 和未知 TUI 的本地/SSH 兼容矩阵。TTY 基础层已形成可重复 smoke：5 个本地 Agent 与 5 个 SSH Agent 均完成启动 + resize，未知 TUI 的本地/SSH 完整协议合约通过；8/10 真实 Agent 格观察到 bracketed 多行 marker，5/10 在 fallback interrupt 前正常退出。provider 快照 4/10 完成，3 格确认真实 tool event；后续 session 层 9 格中，本地 Claude、本地 Codex、本地 Pi、SSH Codex 已用明确会话身份和上下文 token 通过。SSH Claude 固定 UUID 身份已落盘但两轮均为 402；SSH Aider history 身份已落盘但 key 无法认证；OpenCode 两端均已进入同一专用 session，但本地 timeout、SSH 凭据错误；SSH Pi 在创建 session 前缺 key，均不计 resume 通过。安装版 Tunara 1.14.0 内的未知 TUI 另已通过 50/50 分屏 resize、后台切回、可见输出复制和中断后 shell 恢复。其余逐 Agent 权限/确认、失败/resume、分屏、后台与复制粘贴证据仍未完成，见 [兼容性基线](./benchmarks/m1-agent-tui-compatibility-2026-07-11.md)。
