...
- [ ] Markdown/MDX 目录、锚点、查找、代码块、表格与源码/预览切换。`.mdx` 现与 `.md` 共用安全惰性阅读路径；预览态 Cmd/Ctrl+F 复用既有查找条，在可见语义文本中高亮、循环前后跳转并滚到当前匹配，当前项使用 accent inset ring 与 `aria-current` 区分，计数通过 polite live region 更新。长目录限制在 `min(35vh, 240px)` 内局部滚动；表格使用 intrinsic width、100% 最小宽度及 96–280px cell bounds。源码层以不接管输入、IME、选择与原生撤销历史的只读 overlay 高亮标题、标记、链接、代码、MDX tag/value；源码/预览切换按文档相对滚动位置映射并恢复，Edit/Preview 使用 roving tabindex、方向键与 Home/End 的标准 tab 模型。仍需真实窄窗像素与纯键盘验收、MDX 混合边界运行复核，故本项保持未完成。
