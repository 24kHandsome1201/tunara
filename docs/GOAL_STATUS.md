...
- [x] Agent 等待确认成为独立状态，不再伪装成“运行中”或“已完成”。Claude 本地/SSH 通过结构化 `PermissionRequest` hook 发出等待事件、`PreToolUse` 恢复运行；Codex 使用仅限运行中会话的成对批准文案 fallback，排除 trust/hooks/login 与松散 Yes/No 误判。会话卡、状态条和全局注意力使用静态 warning 语义与中英文“等待确认”，不自动批准；SSH 失败/断开仍保持更高优先级。生命周期覆盖 running → waiting → running/idle/exit、前后台 unread、OSC/socket 解析和提示消失后的恢复。
