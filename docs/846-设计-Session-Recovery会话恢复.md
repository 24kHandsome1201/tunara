...
> 背景：Otty 的体验启发是“关闭后重开，每个 pane 的精确位置和滚动位置都恢复”。Tunara 当前已经有 session metadata 和部分 UI layout 持久化，但还没有把工作台现场恢复到足够接近关闭前的状态。

## 目标

把 Tunara 的恢复能力从“会话列表恢复”升级到“工作台现场恢复”：
...
用户感知目标：重开 Tunara 后，窗口看起来应尽量像刚才关闭前那一刻，而不是只剩一个重新启动的空 shell。
...
- 不做无限 scrollback 持久化。
- 不自动重启所有后台 session。
- 不把 Tunara 扩成 agent 管理平台、agent launcher 或对话历史系统。
...
Tunara 不应该偷偷把 agent 当作后台任务重启。v1 只做可恢复意图：
...
```text
[tunara restored snapshot, new shell started below]
```
...
这个方案的重点不是把 Tunara 做成 tmux 或后台 agent 平台，而是补上现代终端最强的“现场感”：关闭重开后，用户回到同一个工作台、同一个 session、同一个 scroll 位置。实现上应先统一 workspace snapshot，再接 xterm serialize，最后补 agent resume intent。这样每一步都能独立合并，也不会破坏当前“带智能侧栏的真实终端”边界。
