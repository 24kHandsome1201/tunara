...
Tunara keeps the existing complete preview/editor path for small UTF-8 files. When that path reports a truncated or oversized file, the file surface offers an explicit, read-only **View beginning** action, similar to `head -n N`.

## User contract

- Safe presets are 100, 500, 1,000 (default), and 2,000 lines.
- Results are rendered as one multiline `<pre>` text node rather than one DOM node per line.
...
- `fs_file_view_head_v1(path, lineLimit, requestId)` for local files;
- `ssh_file_view_head_v1(binding, path, lineLimit, requestId)` for SFTP;
