...
- 标准布局为 272px 侧栏、弹性终端、320px 检查器。
- 检查器一次只呈现一种主状态；文件预览占满右栏内容区并取代列表，关闭后无损返回原列表状态。
- 辅助面板是否停靠由终端可用宽度决定（单 pane 至少 480px，每个分栏列 280px），而不是固定的 720/900px 断点。空间不足时检查器先让路，然后是侧栏。
...
- 不使用玻璃模糊作为外壳层级，也不把透明模糊当成侧栏/标题栏/检查器的深度手段。用户可选的终端列墙纸（默认关）只画在字形后面，见 [TERMINAL_WALLPAPER.md](./TERMINAL_WALLPAPER.md)。
