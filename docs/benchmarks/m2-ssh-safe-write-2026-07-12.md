...
- SSH safe-write、reconcile、Markdown 与 editor 定向测试通过；当前完整 Node 538 项中 535 通过、3 个 loopback sandbox 用例 skipped。合同锁定本地与 SSH 接线、create-new、二次 fingerprint、flush/mode/sync/shutdown、原子替换、“不得删除目标”、unknown token 严格解析、草稿跨 PTY 重挂载、reload rejection 分类与草稿保留、终端退出清除失效 PTY、SSH 失败原因边界、源码/预览滚动映射、MDX 静态边界和 UI 禁止盲目重试。
