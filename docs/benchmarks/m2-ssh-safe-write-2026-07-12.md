...
- `cargo test --manifest-path src-tauri/Cargo.toml`：159/159 通过，2 个需要真实 SSH 环境的测试按条件忽略；新增覆盖替换结果的 saved/unchanged/conflict 三分支、同尺寸内容冲突、mode 不一致、8 路同路径并发峰值为 1 和释放锁清理。
