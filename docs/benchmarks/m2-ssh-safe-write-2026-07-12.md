...
- SSH safe-write、reconcile、Markdown 与 editor 定向测试通过；当前完整 Node 532 项中 529 通过、3 个 loopback sandbox 用例 skipped。合同锁定本地与 SSH 接线、create-new、二次 fingerprint、flush/mode/sync/shutdown、原子替换、“不得删除目标”、unknown token 严格解析、草稿跨 PTY 重挂载和 UI 禁止盲目重试。
...
- `pnpm build` 生成独立 `FilePreview` chunk 24.29 kB（gzip 7.66 kB）；主 `App` chunk 为 467.95 kB（gzip 126.89 kB），相较上一轮静态接线的 487.59 kB 主 chunk 减少 19.64 kB，证明编辑器与 Markdown parser 已离开首屏静态模块图。
