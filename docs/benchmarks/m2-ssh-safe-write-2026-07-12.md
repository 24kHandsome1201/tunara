...
- M2 产品验收所需的一次性 release-lock 故障控制已由非默认 Cargo feature `m2-safe-write-benchmark` 与独立 Tauri plugin 双重隔离：默认编译图不包含模块，主 command handler 不注册该命令；feature 构建只允许存活 SSH session 和 `/tmp/tunara-m2-safe-write-benchmark-*` 单层文件，按 `(ptyId,path)` 一次性消费，不接收 shell、主机、凭证或任意故障阶段。默认 Rust 168 项通过，feature Rust 170 项通过（均另有 5 项真实环境 ignored），feature 严格 clippy 与格式门通过。该入口尚未完成隔离 bundle 的 GUI 驱动与产物负向验证，因此不据此关闭 GUI 门。
