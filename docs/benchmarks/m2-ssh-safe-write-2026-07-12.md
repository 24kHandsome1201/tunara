...
结果：原事务返回 `OutcomeUnknown { expected_mode: 0640, cleanup_pending: true }`；新连接调用从 Tauri command 抽出的同一 `reconcile_text_write_with_sftp` 生产核心，得到 Saved。断线留下的专属 temp/lock residue 被测试明确观察后，仅在随机 `/tmp/tunara-m2-status-loss-*` 隔离目录内清理。该证据关闭“真实 replace 已执行但状态丢失”的后端门，不冒充 GUI 重连点击或自动清理闭环。

## 尚未关闭

- GUI 重连后触发 reconcile，并自动清理本次 unknown 所属的 temp/lock residue。
