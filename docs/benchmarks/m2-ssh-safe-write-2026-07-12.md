...
- SSH safe-write、reconcile、Markdown 与 editor 定向测试通过；当前完整 Node 535 项中 532 通过、3 个 loopback sandbox 用例 skipped。合同锁定本地与 SSH 接线、create-new、二次 fingerprint、flush/mode/sync/shutdown、原子替换、“不得删除目标”、unknown token 严格解析、草稿跨 PTY 重挂载、源码/预览滚动映射和 UI 禁止盲目重试。
...
- `pnpm build` 生成独立 `FilePreview` chunk 26.00 kB（gzip 8.17 kB）；主 `App` chunk 仍为 467.95 kB（gzip 126.89 kB），相较静态接线的 487.59 kB 主 chunk 减少 19.64 kB，证明新增滚动与键盘逻辑仍留在按需 chunk，未回流首屏静态模块图。
