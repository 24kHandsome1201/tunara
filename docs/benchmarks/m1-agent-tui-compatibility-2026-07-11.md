...
- 未知 TUI 已通过可见输出写入确认；SSH Codex、本地 Pi、本地 Claude、本地/SSH OpenCode 与 SSH Aider 已完成受控 token 的系统剪贴板复制→普通粘贴精确回环且未提交。仍需其余 Agent 复制、普通粘贴与 bracketed paste。
- 本地 Claude、本地/SSH OpenCode 与 SSH Aider 已用 `/exit` 和真实 shell marker 通过正常退出；仍需 Codex 的稳定正常退出命令，以及其他 Agent 退出后的普通 shell 恢复。
