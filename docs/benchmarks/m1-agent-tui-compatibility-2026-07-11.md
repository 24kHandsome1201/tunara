...
| 本地 Aider 安全边界与退出 | 在 `/tmp` 以 `uvx --from aider-chat aider --no-git` 启动，依赖安装期间保持 horizontal 50/50；无 key 时明确拒绝 OpenRouter 登录/建号和打开文档，未进入 provider 调用，随后恢复 `/tmp` 普通 zsh。Finder 后台往返保持 split/session，真实快照记录 `TUNARA_LOCAL_AIDER_SHELL_OK` | 通过安全拒绝、后台与 shell 恢复；不宣称 clipboard/resume |
...
| SSH Pi 安全边界与退出 | `de-netcup /root` 通过固定 `npx` 包启动 Pi 0.79.4，horizontal 50/50 保持；TUI 明确显示 `No models available` 与 `/login` 指引，不伪报 provider 成功。空 prompt 发送 `Ctrl+D` 后恢复 `root@de-netcup:~#`，真实快照记录 `TUNARA_SSH_PI_SHELL_OK` | 通过启动、失败边界、分屏与退出；不宣称 clipboard/resume |
| npx Pi Agent 身份 | 安装版未把 scoped、带版本的 `@earendil-works/pi-coding-agent@0.79.4` 映射为 Pi，外层仅显示普通“运行”；`c5d4497` 只读取 npx 首个包位并显式映射官方包，`npx left-pad pi` 负例不误报 | 源码 87 项定向、429 项全量、typecheck、lint、build 通过；新 bundle 实机复验待完成 |
...
- Claude 当前组织策略下的真实人工确认，以及 Pi/OpenCode/Aider 本地与 SSH OpenCode 的首次权限或等价安全边界；Codex 本地目录信任、Pi 本地拒绝持久信任、SSH Pi 缺模型失败边界、SSH Aider 拒绝外部账号创建均已观察。
...
- 本地/SSH Codex、本地/SSH Pi、本地 Claude、本地/SSH OpenCode 与 SSH Aider 已用各自正常退出路径和真实 shell marker 通过；仍需其他 Agent 退出后的普通 shell 恢复。
