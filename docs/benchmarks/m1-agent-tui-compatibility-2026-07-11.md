...
- 生产 provider 恢复后的 SSH Claude、SSH Pi、本地/SSH OpenCode 与本地/SSH Aider resume，以及 Tunara UI 内的恢复端到端证据；其中 SSH Pi 的受控 loopback provider 协议/session 上下文重放已通过，但真实外部 provider 与产品 UI 仍未验证。本地 Claude、本地 Codex、本地 Pi、SSH Codex 已用明确会话身份和上下文 token 通过。
