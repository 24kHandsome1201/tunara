...
- 10 类按键的传输探针已在未知 TUI 本地/SSH 全部自证，历史 harness 有 9/10 真实 Agent 格完成发送；仍需逐 Agent 的补全/选择/历史/取消/多行编辑语义结果，以及用当前 provider 状态重测早期提前退出的 SSH Claude。
...
- 侧栏与 Inspector 展开后的最小几何仍需逐 Agent 补证；已有分屏存活不能替代该更窄布局门。
