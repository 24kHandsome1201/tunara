...
provider 快照层有 4/10 完成真实响应：本地 Claude、本地 Codex、SSH Codex、SSH Aider；其中本地 Claude、本地 Codex、SSH Codex 有结构化 tool event 和完成 marker。session 层 6 格中，本地 Claude、本地 Codex、SSH Codex 按明确 UUID 恢复并保留上下文 token；SSH Aider 的显式 history 身份已落盘，但当前 key 无法认证；OpenCode 两端均确认进入同一专用 session，但本地两轮 timeout、SSH 两轮返回凭据错误。这三格都不计 resume 通过。安装版 Tunara 1.14.0 中的未知 TUI 还完成了真实分屏 resize、后台切回、快速复制和中断后 shell 恢复。完整 GOAL 仍要求逐 Agent 本地/SSH 证据，因此 M1 状态保持未完成。
...
- Pi 与 SSH Claude 的 resume，以及 provider 恢复后的本地/SSH OpenCode 与 SSH Aider resume；本地 Claude、本地 Codex、SSH Codex 已用明确 ID 和上下文 token 通过。
