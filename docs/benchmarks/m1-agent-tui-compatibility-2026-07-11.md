...
| compound/uvx Agent 身份 | 安装版只看第一个 token，未把 `cd /tmp && uvx --from aider-chat aider --no-git` 识别为 Aider，并把交互回答 `n` 错作 session title；`d8d6a2b` 增加引号感知的 shell segment/word 解析、路径/`env`/`exec` 包装与 `uvx --from` 入口识别，同时保持 `echo aider`、引号内 Agent 名称不误报 | 源码 87 项定向、425 项全量、typecheck、lint、build 通过；新 bundle 实机复验待完成 |
| Titlebar tab/关闭语义 | 安装版辅助技术树把“选择会话”和“关闭会话”压成一个按钮；尝试切换 Pi tab 实际进入“再次点击确认关闭”，未二次确认且会话保持运行。`2c7350f` 将容器改为具名 tablist，选择使用独立 tab，关闭使用独立 button，并让安静隐藏的关闭按钮在键盘 focus 时可见 | 源码 2 项定向、427 项全量、typecheck、lint、build 通过；内置浏览器无 IAB backend，新 bundle 视觉复验待完成 |
...
- 本地/SSH Codex、本地 Claude、本地/SSH OpenCode 与 SSH Aider 已用 `/exit` 和真实 shell marker 通过正常退出；仍需其他 Agent 退出后的普通 shell 恢复。
