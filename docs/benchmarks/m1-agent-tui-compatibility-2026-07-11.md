...
- 未知 TUI 已通过可见输出写入确认；本地/SSH Claude、Codex、Pi、OpenCode 与 Aider 均已完成受控 token 的系统剪贴板复制→普通粘贴精确回环且未提交，并通过原生两行预警、确认、精确输入、不自动提交和取消边界。SSH Claude 当前 bundle 还以真实远端 prompt 与独立 shell marker 补齐正常退出。此前“后端已开但 pane 永久重连”已定位为 Fast Refresh 的 `initRef` 单向锁并修复，不再列作 SSH IPC 阻塞。
- 本地/SSH Claude、Codex、Pi、OpenCode 与 Aider 均已用各自正常退出或安全拒绝路径恢复普通 shell；所有正常退出格均有真实 shell marker，SSH Claude 当前 bundle 的 marker 还由独立 SSH 读回。
