...
provider 快照层有 4/10 完成真实外部响应：本地 Claude、本地 Codex、SSH Codex、SSH Aider；其中本地 Claude、本地 Codex、SSH Codex 有结构化 tool event 和完成 marker。后续 session 层已覆盖 10/10 本地/SSH Agent 格，其中本地 Claude、本地 Codex、本地 Pi、SSH Codex 完成真实外部 provider 上下文 resume；本地 Pi 用固定 UUID 产生 18 个 JSON event，first/resume marker 均命中，证明 provider 状态已较早期“缺 key”快照发生漂移。本地/SSH Aider 的显式 history 身份已落盘，但当前 key 无法认证；SSH Claude 固定 UUID 身份已落盘，但两轮均是 402；OpenCode 两端均确认进入同一专用 session，但本地 timeout、SSH 凭据错误；SSH Pi 的生产 provider 探针仍在创建 session 前缺 key。受控探针已在真实 `de-netcup` 用 Pi 0.79.4、固定 session UUID 与仅绑定 `127.0.0.1` 的确定性 Chat Completions 端点完成两轮上下文重放；当前 Tuna Debug bundle 还已在同一真实 SSH pane 中通过产品恢复按钮重放原 cwd、session identity、安全姿态和上下文。这仍只证明 loopback provider 协议/session 与 Tunara 产品恢复链，不计入 4/10 外部 provider 响应；见 [SSH Pi 协议层摘要](./raw/m1-agent-tui-2026-07-11/ssh-pi-loopback-resume-summary.json) 与 [SSH Pi 产品恢复摘要](./raw/m1-agent-tui-2026-07-11/ssh-pi-product-resume-summary.json)。安装版 Tunara 1.14.0 中的未知 TUI 还完成了真实分屏 resize、后台切回、快速复制和中断后 shell 恢复。逐 Agent 的终端交互语义已经 10/10 通过；外部 provider 环境格继续如实保留，但按 M1 合同与 GOAL 的明确非目标不再阻塞 M1。
...
| 脱敏摘要 | [TTY summary](./raw/m1-agent-tui-2026-07-11/result-summary.json) · [provider summary](./raw/m1-agent-tui-2026-07-11/provider-summary.json) · [session summary](./raw/m1-agent-tui-2026-07-11/session-summary.json) · [SSH Pi loopback resume](./raw/m1-agent-tui-2026-07-11/ssh-pi-loopback-resume-summary.json) · [SSH Pi product resume](./raw/m1-agent-tui-2026-07-11/ssh-pi-product-resume-summary.json) · [app runtime summary](./raw/m1-agent-tui-2026-07-11/app-runtime-summary.json) |
...
| SSH Bash Agent 身份 | 安装版复现裸 OSC 133 `C` 让命令块混入 `root@de-netcup:~#`，导致 Aider 未被识别并沿用旧 Codex 可恢复身份；`ca79bf3` 改为优先使用真实提交输入，OSC 自带 payload 仍优先，终端缓冲仅兜底 | 当前 Debug bundle 在同一真实 `de-netcup` pane 提交 `cd /tmp && /usr/local/bin/aider --version`；运行时精确显示 `Aider · 运行中`，结束后回到 `root@de-netcup: /tmp`，未混入 prompt，通过 |
...
| 跨 Agent 恢复身份 | 安装版在同一 pane 先退出 Claude、再运行并退出 OpenCode 后，界面错误显示 `OpenCode 可恢复`，但持久化 `agentResume` 仍是旧 Claude UUID，恢复按钮未启动 OpenCode。`a956004` 在 Agent 身份切换时清除不匹配 resume，并让历史脏快照按真正 resume agent 显示 | 当前 bundle 在带旧 Pi 恢复意图的同一 SSH pane 启动 Aider；运行时只显示 Aider，退出后该 pane 不再显示 Pi/Aider 假恢复，另一会话的 Pi 恢复卡保留，证明只清理当前会话，通过 |
...
| 精确 session id 的 Agent 命令隔离 | 安装版在记录 Codex 精确 session id 后，持久化 `agentResume.agent` 为 Codex，但 source command 错沿用 `claude --permission-mode plan`；`9202a37` 只允许同 Agent 的旧命令，且只接受与当前 Agent 探测结果一致的 `lastCommand` | 当前 Debug bundle 在真实 SSH `/tmp` 以 `codex --sandbox read-only --ask-for-approval never` 启动并正常退出；点击恢复无需回车立即进入启动中/加载中，命令块精确显示 `codex --sandbox read-only --ask-for-approval never resume`，不含 Claude 命令且保持 SSH `/tmp`，通过 |
...
以下项目用于持续观察真实外部环境，不能用未知 fixture 或单次启动冒充通过；它们不属于 Tunara 的模型账号或 provider 交付责任：
