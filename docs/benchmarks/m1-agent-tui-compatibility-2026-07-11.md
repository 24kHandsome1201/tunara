...
| 本地 Pi 状态语义 | Pi 0.79.4 启动后被识别；就绪画面保留 `$0.000 (sub) 0.0%/272k (auto)  gpt-5.5 •`，执行无 provider 依赖的 `!sleep 2` 时同时出现 `Running... (escape/ctrl+c to cancel)`，完成后该行消失 | 旧安装版误报运行中；源码 `118a1c5` 已改为忙碌优先并加入确定性回放，待新 bundle 复验 |
...
- 未知 TUI 与 SSH Codex 已通过 Tunara 内实际分屏存活；本地 Pi 已完成应用内启动及真实忙碌/就绪画面取证。仍需其余 Agent 的分屏、侧栏/Inspector 展开后的最小几何，以及新 bundle 对 Pi 状态转换的复验。安装版还暴露“SSH 分屏新 pane 误落本机”问题，源码已修复，待新 bundle 复验。
- 未知 TUI 与 SSH Codex 已通过窗口后台切回后的 session、split 与 alternate-screen 所有权；仍需其余 Agent 证据及后台期间输入流。
- 未知 TUI 已通过可见输出写入确认；SSH Codex 已完成受控 token 的系统剪贴板复制→普通粘贴精确回环且未提交。仍需其余 Agent 复制、普通粘贴与 bracketed paste。
