...
- 10 类按键的传输探针已在未知 TUI 本地/SSH 和 10/10 真实 Agent 格完整发送；SSH Claude 还完成模式循环、斜杠菜单上下选择与取消、Ctrl+R 历史搜索打开与取消、双行编辑与取消清空语义。其余 9 个真实 Agent 格仍需对应的补全、选择、历史、取消和多行编辑语义结果。
