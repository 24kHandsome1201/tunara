...
- 未知 TUI、SSH Codex 与本地 Pi 已通过窗口后台切回后的 session 与 split 保留；仍需其余 Agent 证据及后台期间输入流。
- 未知 TUI 已通过可见输出写入确认；SSH Codex 与本地 Pi 已完成受控 token 的系统剪贴板复制→普通粘贴精确回环且未提交。仍需其余 Agent 复制、普通粘贴与 bracketed paste。
