...
| SSH Claude 首次权限、恢复与多行安全 | 既有 `/root`、`/tmp` 两次 `No, exit` 证明安全拒绝与后台保留。当前源码又从持久化 split 冷启动：后端 `ssh opened id=2` 后前端 open Promise 以 `disposed=false` 返回；受控 HMR 则在修复后连续完成 SSH `2→4→6` 和 local `1→3→5` PTY 重建，辅助树有两个独立 `Terminal input`、无连接遮罩，Tunara pane 写入的 `TUNARA_SSH_SHELL_BASELINE` 可由独立 SSH 读回。在隔离 `/tmp/tunara-claude-smoke` 信任并启动 plan mode 后，进程明确位于 `pts/0` 且带 Tunara `--plugin-dir`；两行受控剪贴准确触发原生 2 行 Cancel/OK 预警，确认后没有 busy hook 或自动 provider 提交 | 通过恢复连接、隔离信任、远端 TTY、行数预警与不自动提交；GPU 截图为黑块，未取得双行可见/取消清空证据，`/exit`/Ctrl+D 也未自然退出，最终仅对测试 PID 发送 SIGINT 并清理目录，因此普通剪贴板回环、正常退出、provider/resume 仍不宣称通过 |
...
- 未知 TUI 已通过可见输出写入确认；本地/SSH Codex、本地 Pi、本地 Claude、本地/SSH OpenCode 与本地/SSH Aider 已完成受控 token 的系统剪贴板复制→普通粘贴精确回环且未提交；本地/SSH Aider、本地/SSH OpenCode、本地/SSH Pi、本地/SSH Codex 与本地 Claude 还已通过原生两行预警、确认、精确输入、不自动提交和取消边界。SSH Claude 当前 bundle 已补上真实恢复连接、隔离信任、2 行预警和不自动提交，但受 GPU 截图黑块与当前窄 pane 取证限制，仍缺双行可见、取消清空、普通剪贴板回环和正常退出证据。此前“后端已开但 pane 永久重连”已定位为 Fast Refresh 的 `initRef` 单向锁并修复，不再列作 SSH IPC 阻塞。
