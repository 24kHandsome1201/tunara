...
- 未知 TUI 已通过可见输出写入确认；本地/SSH Codex、本地 Pi、本地 Claude、本地/SSH OpenCode 与本地/SSH Aider 已完成受控 token 的系统剪贴板复制→普通粘贴精确回环且未提交；本地/SSH Aider、本地/SSH OpenCode 与本地/SSH Pi 还已通过原生两行预警、确认、精确输入、不自动提交和 `Ctrl+C` 取消的 bracketed paste 语义。仍需本地/SSH Codex、本地/SSH Claude 的 bracketed paste，以及 SSH Claude 的普通剪贴板回环。
