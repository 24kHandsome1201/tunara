...
10. Trigger a failed disposable SSH connection and verify diagnostics contain only typed stage/status/code values—never raw backend errors, passwords, paths, host comments, or transport identifiers. Copy the de-identified report and repeat the canary check.
...
| Scope declarations | Visit Changes, Files, Preview, Transfers, and Forwarding. Read the line below the tabs. Switch between two sessions, then reconnect one SSH session. | Every panel visibly states its title and applicable scope. Transfers stays on the logical Session; Forwarding and connected remote Files use the active Connection. Session and binding changes remount the scoped panel and invalidate pending results. |
...
| Optional terminal background | Settings → Terminal → Terminal background off (default). Enable a built-in texture, then turn it off. | Off keeps the solid theme canvas. On is announced as a switch; blur/dim sliders expose values. Reduce Transparency keeps the solid canvas. Custom photo controls are keyboard reachable. |
