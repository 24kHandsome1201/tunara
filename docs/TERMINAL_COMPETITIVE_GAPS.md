...
| 命令块 + Quick Select + 安全粘贴 | 有 | 各家只做其中一两项 |
| ~30 MB、无账号、无遥测 | 有 | Warp 安装包大约 150–230 MB，且偏云 |

侧栏会话卡已经显示 Git 分支。全局唤起（⌘⇧T）、OSC 9 / OSC 99 / OSC 777 通知、OSC 9;4 进度条、可选 OSC 52 写剪贴板、连字、正则搜索、本地拖放插入路径、有上限的滚屏/命令块导出、⌘⇧U 跳到最近 Attention，代码里都在。不要把它们再报成缺口。
