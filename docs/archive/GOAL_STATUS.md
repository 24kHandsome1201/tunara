...
`M0 Phase 1 真实验收`、`M1 Terminal + SSH 性能与乱码稳定性`、`Phase 2 Markdown / 单文件轻编辑` 与 `Phase 3 Workspace-bound Preview` 均已完成。Agent 产品范围保持轻量：检测、Attention 与按 CLI 可用的 resume；不提供持久事件存储、全文搜索或富 payload Timeline。已移除的完整原型保存在远端 `archive/agent-timeline-v1`，只作为历史参考。当前版本不读取、也不自动删除 v1.16 曾写入的本地 Agent 历史，应用内不再提供旧数据清理入口。
...
| Phase 3 Workspace Preview | 已完成 | 完整来源键与 window generation 隔离覆盖 WebView lifecycle、同源历史、安全重启和 SSH tunnel；独立 Preview 窗口没有 app command 权限 | 保持回归；不自动进入新阶段 |
| 轻量 Agent 辅助 | 当前范围 | Claude Code、Codex、Cursor、OpenCode 优先；其他 catalog 项基础识别。旧 snapshot 的已移除 Inspector 页签安全回退 Changes | 不引入持久历史、搜索、富 payload 或聊天壳 |
