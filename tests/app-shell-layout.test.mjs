...
test("compact app shell overlays both auxiliaries without shrinking the terminal", () => {
  for (const viewportWidth of [576, 640, 719]) {
    const layout = resolveAppShellLayout({ ...defaults, viewportWidth, terminalColumnCount: 2 });
...
test("dock decisions use terminal budget instead of discontinuous viewport breakpoints", () => {
  ...
  const before = resolveAppShellLayout({ ...defaults, viewportWidth: 899, terminalColumnCount: 2 });
  const after = resolveAppShellLayout({ ...defaults, viewportWidth: 900, terminalColumnCount: 2 });
...
test("terminal width never falls when a panel changes from overlay to docked", () => {
  for (const [terminalColumnCount, boundary] of [[1, 1072], [2, 1157]]) {
    const widths = [boundary - 2, boundary - 1, boundary, boundary + 1, boundary + 2]
      .map((viewportWidth) => resolveAppShellLayout({ ...defaults, viewportWidth, terminalColumnCount }).terminalWorkspaceWidth);
    for (let i = 1; i < widths.length; i += 1) {
      assert.ok(widths[i] >= widths[i - 1], `${terminalColumnCount}-column terminal width regressed at ${boundary}`);
    }
  }
});

test("single and vertical terminals use one-pane horizontal budget", () => {
  const layout = resolveAppShellLayout({ ...defaults, viewportWidth: 1000, terminalColumnCount: 1 });
...
test("four side-by-side panes reserve four terminal columns before docking auxiliaries", () => {
  const layout = resolveAppShellLayout({ ...defaults, viewportWidth: 1280, terminalColumnCount: 4 });
  assert.equal(layout.minimumTerminalWorkspaceWidth, MIN_TERMINAL_PANE_WIDTH * 4 + SPLIT_HANDLE_WIDTH * 3);
  assert.equal(layout.terminalWorkspaceWidth, MIN_TERMINAL_PANE_WIDTH * 4 + SPLIT_HANDLE_WIDTH * 3);
  assert.equal(layout.sidebarOverlay, true);
  assert.equal(layout.panelOverlay, true);
});
...
test("opening a compact drawer closes its overlapping sibling", () => {
  const compact = { ...defaults, viewportWidth: 640, terminalColumnCount: 2 };
