...
test("starts and stops loopback-only local forwarding with requested and actual ports", async () => {
  ...
  mockIPC((command, payload) => {
    ...
    if (command === "ssh_dynamic_forward_list") return [];
...
test("starts dynamic SOCKS5 forwarding without exposing Remote or Agent modes", async () => {
  const calls: Array<{ command: string; payload: Record<string, unknown> }> = [];
  ...
  expect(screen.queryByRole("radio", { name: /remote/i })).toBeNull();
...
test.each(["resolve", "reject"] as const)("stale generation list calls cannot affect the current panel when they %s", async (settlement) => {
  ...
  mockIPC((command, payload) => {
    if (command !== "ssh_local_forward_list" && command !== "ssh_dynamic_forward_list") {
      throw new Error(`unexpected command: ${command}`);
    }
