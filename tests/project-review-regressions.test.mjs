...
test("settings exposes the signed updater flow and restart permission", () => {
  ...
  const appLifecycle = read("src/app/app-lifecycle.ts");
  const lib = read("src-tauri/src/lib.rs");
  const defaultCapability = JSON.parse(read("src-tauri/capabilities/default.json"));
  ...
  assert.match(appLifecycle, /await relaunch\(\)/);
...
test("follow-up review fixes polish dense UI surfaces", () => {
  ...
  assert.match(settings, /data-color-scheme-preview="window"/);
  assert.match(settings, /data-preview-region="sidebar"/);
  assert.match(settings, /data-preview-region="terminal"/);
  assert.match(settings, /data-preview-region="panel"/);
