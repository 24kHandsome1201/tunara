...
if set -q TUNARA_SESSION_ID
  ...
  function _tunara_agent_write_hooks --argument-names sid agent config_dir
    ...
    printf '{"hooks":{"SessionStart":[{"matcher":"startup|resume","hooks":[{"type":"command","command":"%s"}]}],"UserPromptSubmit":[{"hooks":[{"type":"command","command":"%s"}]}],"PreToolUse":[{"hooks":[{"type":"command","command":"%s"}]}],"PermissionRequest":[{"hooks":[{"type":"command","command":"%s"}]}],"PostToolUse":[{"hooks":[{"type":"command","command":"%s"}]}],"PostToolUseFailure":[{"hooks":[{"type":"command","command":"%s"}]}],"Stop":[{"hooks":[{"type":"command","command":"%s"}]}],"StopFailure":[{"hooks":[{"type":"command","command":"%s"}]}],"Notification":[{"matcher":"idle_prompt","hooks":[{"type":"command","command":"%s"}]}]}}' $idle $busy $busy $wait $busy $busy $stop $stop $idle >"$runtime/hooks/hooks.json"
