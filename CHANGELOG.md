...
- 修复 SSH 文件表头与行的“修改时间”列错位；240–280px 窄面板会隐藏时间列，为文件名、选择框和操作入口保留空间。
- 远程位置与传输操作改为稳定的分组工具栏，进行中传输使用固定宽度的图标与数字 badge，不再因“上传文件夹”任务换行或改变工具栏高度。
- SSH 断线会区分有缓存与无缓存：有缓存继续显示只读树，无缓存直接给出重连提示，不再停在误导性的加载状态；目录空态和读取错误态改为平衡的居中布局并提供重试。
...
- ⌘⇧U 跳到最近需要处理的会话（失败 SSH、未读 Agent 工作、失败命令）；没有目标时给出提示。
- 右键菜单和命令面板可把滚屏或命令块输出另存为文件，上限 2000 行 / 256 KiB。
- 终端右键菜单识别光标下的命令块：显示运行状态、退出码与耗时，可复制命令/输出、滚动到命令，或把命令回填到输入行（不自动执行）。
- 非活跃会话的命令完成提醒附带耗时；运行 ≥15 秒的长命令在窗口处于后台时还会请求 Dock 提醒（受铃声通知开关控制）。
- SSH 文件上传完成后可直接预览远端文件；传输中心的已完成上传也提供同一入口。
- 拖放到文件树中的具体目录，并记住每个主机上次的本地下载目录。
- 大文本/日志支持查看结尾（`tail`），前台预览按修订/内容轻量刷新。
- 预览可用方向键或 `j`/`k` 在同目录文件间切换。
- 每个 SSH 主机可收藏和记住最近的远端路径。
- 文件面板可跟随终端 OSC 7 工作目录。
- SSH 增加仅回环的 RemoteForward（反向端口转发）。
...
- 重写若干别扭的 SSH / UI 中英文案。
- 将 `tauri-plugin-log` 升到 2.9，与 npm `@tauri-apps/plugin-log` 对齐，解除 `tauri build` 的插件 minor 不匹配拦截；该升级同时移除未使用的 `rkyv` 0.7，**RUSTSEC-2026-0235** 不再出现在依赖图中。

## [1.17.1] - 2026-08-01

### 新功能
- SSH 文件面板新增上传、覆盖确认、真实进度、取消及失败恢复提示；覆盖上传通过 OpenSSH SFTP 原子替换，保留原权限并拒绝目录、符号链接和并发权限变化。
- Pure Mode 原生全屏退出控件支持横向拖动，并新增“显示纯净模式文件按钮”持久化设置；关闭按钮后仍可通过快捷键和命令面板打开文件面板。

### 产品与体验
- 完成终端搜索、SSH 状态横幅、文件菜单、Quick Select、Preview Inspector、会话排序与状态播报、弹窗滚动及复制反馈的响应式与无障碍修复。
- 统一小字号与语义色 token，改善亮暗主题对比度、键盘菜单入口、焦点恢复、触控热区以及窄面板操作布局。

### 稳定性与安全
- SSH 上传临时文件使用高熵随机名称；失败后不再按路径删除无法证明归属的远端文件，并以本地化提示准确报告可能残留的路径。
- SSH 下载增加 100 MiB 前置限制、进行中防重复状态和可操作错误文案；Pure Mode 文件操作反馈保持全局可见并归属于发起会话。

## [1.17.0] - 2026-07-24

### 新功能
- 文件面板升级为工作区文档标签：本地与 SSH 文件可与终端标签并列打开，支持按名称或修改时间排序、多文件草稿保护，以及安全的“在终端中打开”和本地 VS Code 入口。
- 新增只读 Notebook 预览，安全展示 Markdown、代码、原始单元格及纯文本/错误输出，不执行代码，也不渲染 HTML、脚本、图片等富输出。
- SSH 连接改为显式选择 Agent、私钥、密码或 keyboard-interactive 认证；密码和口令只在单次连接内存中使用，不写入主机配置或工作区快照。

### 产品与体验
- 持久化 Agent Timeline、搜索和富载荷界面退出当前产品范围；运行时 Agent 检测、提醒、恢复和最近活动继续保留，设置中仍可检查并删除旧版遗留数据。
- Pure Mode、终端命令块、文件/设置/标题栏导航及右键菜单补齐可发现入口、键盘操作、焦点回收、状态播报和中英文文案。
- 大目录启用虚拟滚动，文件预览高亮与查找增加防抖，页面隐藏时暂停 Preview 轮询，降低长列表和后台窗口开销。

### 稳定性与安全
- SSH 重连按 generation 隔离旧 channel，并在线性化发布点拒绝过期连接；失败的替换不会中断仍可用的会话，远程安全写锁遇到无法原子判定的陈旧状态时改为 fail closed。
- 修复远程文件面板无法浏览初始 cwd 上级、初次文件读取失败不可恢复、分栏拖拽卸载残留全局状态，以及多处终端/UI 焦点和状态同步问题。
- 修复自定义终端字体族编码顺序，先转义反斜杠再转义引号，避免构造不完整的 CSS 字体描述。
- 更新 ESLint 与 Node 类型定义，并保持 Tauri npm/Cargo 版本耦合、CodeQL、依赖审计及跨平台 CI 门禁。

## [1.16.0] - 2026-07-21

### 新功能
- 分栏终端升级为最多四个 pane 的递归布局，支持在任意 pane 右侧或下方继续拆分，并按实际几何方向切换焦点。
- 新增纯终端展示模式，可通过快捷键或命令面板进入，隐藏应用壳层、状态信息和辅助浮层，同时保持既有 PTY 挂载与会话状态。

### 设计与体验
- 默认终端工作区移除顶部 Agent 状态栏，以及带命令名和完成勾选的命令块卡片，使终端内容直接从 pane 顶部开始。

### 稳定性与性能
- 在持续高输出下按全局压力重建共享 WebGL atlas，并保留限频与静默补偿机制，降低多 pane 长时间输出后的字符渲染损坏风险。

## [1.15.0] - 2026-07-15

### 新功能
- 新增按工作区与任务隔离的 Agent Timeline，以虚拟列表承载长历史，并在追加、向前翻页和动态行高变化时保持稳定滚动锚点。
- Timeline 支持延迟加载富载荷、结构化工具调用与 Markdown 内容展示；大载荷不会进入轻量事件头或拖慢首屏。
- 新增持久层全文搜索索引与筛选体验，可在命令面板和 Timeline 中快速定位 agent 事件，同时保持工作区信任边界。
- 完成文件预览能力扩展：支持本地及 SSH 文本安全写入、Markdown 阅读、预览历史与导航、可信缩放/视口、远端隧道和源绑定截图。

### 稳定性与性能
- 强化本地与 SSH 终端的流控、渲染恢复、工作区恢复和 agent resume 生命周期，并补齐 Codex、Claude、Pi、OpenCode、Aider 的交互兼容验证。
- 增加 Timeline、终端渲染、SSH 高延迟/高吞吐和多 Agent TUI 的可复现 benchmark 与回归门槛。

## [1.14.0] - 2026-07-11

### 新功能
- 命令块历史与真实终端滚屏对齐：保留仍可导航的最近 240 条记录，自动淘汰已离开 scrollback 的 marker，最近 5 条之外通过可滚动的“更早 N 条”菜单访问，当前输出不再重复显示。
- 工作区恢复完成后延迟检查签名发布通道；仅在确有新版本时展示低打扰通知，点击直接进入“设置 > 应用”并自动复核更新，开发模式、网络失败和无更新都保持安静。
- 会话右键菜单新增“选择动物吉祥物”，会自动打开概览并聚焦当前选择，让已加入的 8 个动物图标更容易发现和使用。

### 设计与体验
- 通知卡片移除与设计系统冲突的 3px 左强调条，继续通过语义图标、颜色和操作按钮表达状态；修复悬停暂停后进度条从头播放、与实际关闭计时不同步的问题。
- 设置页的当前分区进入统一 UI 状态，来自快捷键、命令面板和更新通知的入口可以准确落到对应分区；长命令历史菜单增加可滚动边界与键盘焦点跟随。
